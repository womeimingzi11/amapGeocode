...
## Introduction ![amapGeocode hex sticker](reference/figures/hexSticker-logo.png)
...
- Unified HTTP handling via [{httr2}](https://httr2.r-lib.org) with
  ...
  rate-limit headers.
- Opt-in request signing through
  ...
  keeps sensitive secrets out of function calls.
- [`getCoord()`](https://womeimingzi11.github.io/amapGeocode/reference/getCoord.md)
  supports `mode = "all"` for multi-match lookups and `batch = TRUE` for
  native AutoNavi 10-at-a-time batching.
- [`getLocation()`](https://womeimingzi11.github.io/amapGeocode/reference/getLocation.md)
  supports native batching plus rich POI, road, intersection, and AOI
  list-columns via the `details` argument when `extensions = "all"`.
- Bulk requests are executed with
  [`httr2::req_perform_parallel()`](https://httr2.r-lib.org/reference/req_perform_parallel.html)
  (curl multi; no new R sessions) and protected by default throttling (3
  requests/second). Tune with
  [`amap_config()`](https://womeimingzi11.github.io/amapGeocode/reference/amap_config.md).
- [`extractAdmin()`](https://womeimingzi11.github.io/amapGeocode/reference/extractAdmin.md)
  traverses every matching parent region, returning tidy parent metadata
  and optional boundary polylines when `extensions = "all"`.
...
For temporary scopes, wrap the workflow with
[`with_amap_signature()`](https://womeimingzi11.github.io/amapGeocode/reference/with_amap_signature.md):
