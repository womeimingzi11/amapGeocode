...
2. Baidu with free verification provices really high request: 300,000/month for each geocode/reverse-geocode, while AutoNavi only provide 6000/month.
...
```{r install by remotes, eval=FALSE, message=TRUE, include=FALSE}
...
What we get from `getCoord` is **JSON** or **XML**. For readability, `extractCoord` is created to get result as `tibble`, [a modern reimagining of the data.frame](https://tibble.tidyverse.org/).
